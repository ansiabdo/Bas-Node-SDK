...
const encrypt = function (data, Mkey) {

    const passPhrase = Mkey;
    const hashAlgorithm = "SHA256";
    const passwordIterations = 2;
    const keySize = 256;
    const saltValue = "aaaa";
    const key = Buffer.from('f2f61cbe28f35df19fa478c60bfb005480f6a0cdff47565c34723084399338d1', "hex")// derp(passPhrase, saltValue, passwordIterations, hashAlgorithm, 32);
    console.log("============== encrypt key:", key)
    console.log("============== encrypt key:", key.toString("hex"))
    const initVector = "dddd888855556666";
    const initVectorBytes = Buffer.from(initVector, "binary");
    console.log("============== encrypt iv:", initVectorBytes)

    const cipher = crypto.createCipheriv(`aes-${keySize}-cbc`, key, initVectorBytes);
    const output = Buffer.concat([cipher.update(data, 'utf8'), cipher.final()]);
    console.log("============== encrypt output:", output.toString('base64'))

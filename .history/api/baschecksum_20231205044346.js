...
class BasChecksum {
	...
	static encrypt(input, key) {
		...
		try {
			let keyAscii = Buffer.from(key, "ascii")
			...
			let key2 = crypto.createHash('sha256', { outputLength: 32 }).update(Buffer.from(key, "ascii")).digest('hex')
			console.log("========= Buffer.from Key ,iv :", Buffer.from(key, "utf8"), Buffer.from(BasChecksum.iv, "utf8"))
			console.log("========= key2:", Buffer.from(key2, "hex").toString("utf8").length, Buffer.from(key2, "hex").toString("hex").length)
			...
			var cipher = crypto.createCipheriv('AES-128-CBC', key2.toString("utf8"), Buffer.from(BasChecksum.iv, "utf8"));

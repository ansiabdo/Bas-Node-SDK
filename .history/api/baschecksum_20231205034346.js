...
class BasChecksum {
	...
	static encrypt(input, key) {
		...
		try {
			...
			let cc = crypto.createHash('sha256').update(key).digest("base64")
			let dd = crypto.createHash('sha256').update(Buffer.from(key, "ascii")).digest('hex')
	...
	static encrypt128(input, key) {
		...
		try {
			...
			console.log("=========128 padded:", padded.toString("hex"))
